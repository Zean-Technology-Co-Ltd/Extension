...
extension UIButton{
    @objc convenience init(imgName name: String, enlargeEdge enlarge: CGFloat = 0) {
        self.init()
        self.setImage(UIImage(named: name), for: .normal)
        self.enlargeEdge(top: enlarge, right: enlarge, left: enlarge, bottom: enlarge)
    }
    
    @objc convenience init(imgName name: String, enlargeEdge enlarge: CGFloat = 0, target: Any, action: Selector) {
        self.init()
        self.setImage(UIImage(named: name), for: .normal)
        self.enlargeEdge(top: enlarge, right: enlarge, left: enlarge, bottom: enlarge)
    ...
    @objc convenience init(title: String, font: UIFont) {
        self.init()
        self.setTitle(title, for: .normal)
        self.titleLabel?.font = font
    }
    
    @objc convenience init(textColor: UIColor, font: UIFont) {
        self.init()
        self.setTitleColor(textColor, for: .normal)
        self.titleLabel?.font = font
    }
    
    @objc convenience init(title: String, textColor: UIColor, font: UIFont) {
        self.init()
        self.setTitle(title, for: .normal)
        self.setTitleColor(textColor, for: .normal)
        self.titleLabel?.font = font
    ...
    @objc convenience init(attributedTitle: String, attributedColor: UIColor, textColor: UIColor, font: UIFont, range: NSRange) {
        self.init()
    ...
    @objc convenience init(title: String, textColor: UIColor, font: UIFont, for state: UIControl.State) {
        self.init()
    ...
    @objc convenience init(title: String, textColor: UIColor, font: UIFont, target: Any, action: Selector) {
        self.init()
        self.setTitle(title, for: .normal)
        self.setTitleColor(textColor, for: .normal)
        self.titleLabel?.font = font
    ...
    @objc func setTitle(title: String, textColor: UIColor, for state: UIControl.State) {
        self.setTitle(title, for: state)
        self.setTitleColor(textColor, for: state)
    }
}
...
extension UIButton{
    func setUrlImage(_ url: String?, for state: UIControl.State) {
        guard let url = url else { return }
        self.sd_setImage(with: URL(string: url), for: state)
    }
    
    func setUrlBackgroundImage(_ url: String?, for state: UIControl.State) {
        guard let url = url else { return }
        self.sd_setBackgroundImage(with: URL(string: url), for: state)
    }
    
    func setImage(imgName name: String, for state: UIControl.State) {
        self.setImage(UIImage(named: name), for: state)
    }
    
    func setBackgroundImage(imgName name: String, for state: UIControl.State) {
        self.setBackgroundImage(UIImage(named: name), for: state)
    }
    
    func dy_localImage(name: String, type: String){
