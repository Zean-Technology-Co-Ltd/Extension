...
extension UIView {
     ...
     * ⚠️ 使用相对布局可能会裁剪失败
     */
    func clipRectCorner(_ roundedRect: CGRect = .zero, direction: UIRectCorner, cornerRadius: CGFloat) {
    ...
    @objc func clipLayerCorner(radius: CGFloat, clipsToBounds clip: Bool = true) {
        clipLayerCorner(radius: radius, borderWidth: 0, borderColor: nil, clipsToBounds: clip)
    }
    
    func clipLayerCorner(radius: CGFloat, borderWidth width: CGFloat, borderColor color: UIColor?, clipsToBounds clip: Bool = true) {
    ...
    @objc func addShadow(offset: CGSize = .zero) {
        addShadow(offset: offset, color: UIColor.black, opacity: 0.1)
    }
    
    @objc func addShadow(offset: CGSize = .zero, color: UIColor = UIColor.black, opacity: Float) {
    ...
    @objc func addCornerShadow(radius: CGFloat = 5) {
        addCornerShadow(offset: .zero, color: .black, opacity: 0.2, radius: radius)
    }
    
    @objc func addCornerShadow(bounds:CGRect = .zero,  offset: CGSize = .zero, color: UIColor = UIColor.black, opacity: Float, radius: CGFloat = 5) {
...
extension UIView {
    ...
    @objc func executeTapActionParametric(_ gesture: UIGestureRecognizer) {
...
extension UIView {
    ...
    func addDashedLine(color: UIColor = UIColor(r: 98, g: 98, b: 98), size: CGSize, lineWidth: CGFloat, name: String = "DashedLine") {
    ...
    func removeDashedLine(name: String? = nil) {
        if name == nil {
