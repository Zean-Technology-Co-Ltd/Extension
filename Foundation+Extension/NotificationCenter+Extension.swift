...
extension NotificationCenter {
    ...
    class func registerNotification(_ observer: Any, _ selector: Selector, name: NNNotificationName) {
        self.default.addObserver(observer, selector: selector, name: NSNotification.Name(rawValue: name.rawValue), object: nil)
    }

    class func postNotification(name: NNNotificationName, userInfo: [AnyHashable : Any]?) {
        self.default.post(name: NSNotification.Name(rawValue: name.rawValue), object: nil, userInfo: userInfo)
    }

    class func remove(_ observer: NNNotificationName) {
        self.default.removeObserver(observer)
    }

    //只适合常驻内存
    class func observe(notificationName: NNNotificationName, block: @escaping (Notification) -> Void) {
